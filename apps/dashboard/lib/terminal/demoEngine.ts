 ...
 * Lightweight: swaps + prices + signals only (chart is DexScreener embed).
 */

import type { Dex, Signal, SwapEvent, WSMessage } from '@/lib/stream/types';
import { SCHEMA_VERSION } from '@/lib/stream/types';

import { SOL_MINT, WATCHLIST, type WatchToken } from './tokens';
...
/** Seed watchlist token prices (no candle flood). */
export function seedDemoPrices(): WSMessage[] {
  initPrices();
  const now = Date.now();
  return WATCHLIST.map((token) => {
    const price = token.refPrice * (0.98 + Math.random() * 0.04);
