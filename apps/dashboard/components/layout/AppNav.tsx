...
export default function AppNav() {
  ...
  return (
    <header
      className={`flex items-center gap-3 h-9 px-3 shrink-0 border-b ${
        onTerminal
          ? 'bg-ds-surface border-ds-border'
          : 'bg-platform-surface border-platform-border'
      }`}
    >
      <Link
        href="/terminal"
        className={`text-[12px] font-semibold tracking-[0.12em] shrink-0 ${
          onTerminal ? 'text-ds-text-primary' : 'text-platform-accent'
        }`}
      >
        SOLARB
      </Link>
              ...
              className={`px-2.5 py-1 text-[11px] font-medium whitespace-nowrap rounded-terminal transition-colors ${
                active
                  ? onTerminal
                    ...
                    : 'bg-platform-accent/12 text-platform-accent'
                  : onTerminal
