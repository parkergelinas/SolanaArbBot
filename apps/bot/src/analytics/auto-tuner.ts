...
export function runAutoTuner(
  ...
  const uptimeSec = (() => {
    const timestamps = events.map((e) => (e as unknown as { timestamp?: number }).timestamp).filter(Boolean);
    if (timestamps.length < 2) return 3600;
    return ((Math.max(...(timestamps as number[])) - Math.min(...(timestamps as number[]))) / 1000) || 3600;
  })();
