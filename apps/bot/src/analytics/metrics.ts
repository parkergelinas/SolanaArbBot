...
export function computeAnalytics(events: readonly JournalEvent[]): AnalyticsSnapshot {
  ...
  const drifts = executions.map(
    (e) => (e.realizedProfitUsd ?? 0) - (e.expectedProfitUsd ?? 0),
  );

  const routeWins: Record<string, { win: number; total: number }> = {};
  for (const e of executions) {
    const key = e.pairLabel ?? 'unknown';
    routeWins[key] ??= { win: 0, total: 0 };
    routeWins[key].total += 1;
    if ((e.realizedProfitUsd ?? 0) > 0) routeWins[key].win += 1;
  }

  const routeWinRates: Record<string, number> = {};
