 ...
 */
export function scoreRoundTripUsd(input: ScoringInput): ScoringResult {
  ...
  const startUi = atomicToUi(pair.forward.request.amount as string, inputDecimals);
  ...
  const grossSpreadBps =
    startUsd > 0 ? Math.round((grossUsd / startUsd) * 10_000) : 0;
...
export function estimateFeesUsd(opts: {
  ...
  const priority =
    ((opts.priorityFeeLamports ?? 50_000) / lamportsPerSol) * opts.solPriceUsd;
  const jito = ((opts.jitoTipLamports ?? 10_000) / lamportsPerSol) * opts.solPriceUsd;
  const slip =
    opts.notionalUsd && opts.slippageBps
