...
export function graduationProgressPct(state: BondingCurveState): number {
...
export function spotPriceSol(state: BondingCurveState): number {
...
export function spotPriceUsd(state: BondingCurveState, solPriceUsd: number): number {
 ...
 */
export function buyTokensForSol(state: BondingCurveState, solLamports: bigint): bigint {
...
export function sellTokensForSol(state: BondingCurveState, tokenAmount: bigint): bigint {
...
export function buyPriceImpactBps(state: BondingCurveState, solLamports: bigint): number {
  if (solLamports <= 0n) return 0;
  const spot = spotPriceSol(state);
  if (spot <= 0) return 0;

  const tokensOut = buyTokensForSol(state, solLamports);
  if (tokensOut <= 0n) return 10_000;
...
export function isEarlyCurveEntry(state: BondingCurveState): boolean {
...
export function isNearGraduation(state: BondingCurveState, minPct = 80, maxPct = 99): boolean {
  const progress = graduationProgressPct(state);
...
export function decodeBondingCurveAccount(mint: string, data: Buffer): BondingCurveState | null {
...
export function freshBondingCurve(mint: string): BondingCurveState {
  return {
    mint,
