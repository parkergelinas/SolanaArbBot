...
const LST_PAIRS: CrossDexScanPair[] = [
  {
    label: 'mSOL/SOL',
...
export const DEFAULT_ADAPTIVE_CONFIG: AdaptiveArbConfig = {
...
export class AdaptiveArbStrategy implements ScannableStrategy {
  ...
  constructor(
    ...
    private readonly capitalTracker: CapitalTracker,
    private readonly cfg: AdaptiveArbConfig = DEFAULT_ADAPTIVE_CONFIG,
  ...
  private evaluateOne(
    opp: CrossDexQuote,
    state: MarketState,
    params: ReturnType<CapitalTracker['getAdaptiveParams']>,
    ctx: StrategyContext,
  ): TradeDecision | null {
  ...
  private nextCoreBatch(n: number): CrossDexScanPair[] {
    const out: CrossDexScanPair[] = [];
