 ...
 */
export function computeDynamicSize(input: SizingInput): SizingResult {
  const {
    spreadBps,
    liquidityUsd,
    routeQualityScore,
  ...
  let ceilingSol: number;
  let ceilingLabel: SizingResult['ceiling'];
  ...
  if (spreadBps >= 72)      spreadFactor = 1.15;  // very wide — take the full ceiling
  else if (spreadBps >= 36) spreadFactor = 1.00;  // healthy spread
  else if (spreadBps >= 20) spreadFactor = 0.85;  // marginal — trade smaller
  else                      spreadFactor = 0.60;  // near break-even — minimum size only
  ...
  const rationale =
    `spread=${spreadBps}bps sol=$${solPriceUsd} ` +
...
export function computeDynamicSizeUi(input: SizingInput): number {
