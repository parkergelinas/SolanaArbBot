...
export function extractSpreadObservations(events, solPriceUsd = 150) {
    const obs = [];
    for (const e of events) {
        if (e.type !== 'rejection' && e.type !== 'decision')
            continue;
        if (!e.rejectionReason)
            continue;
        // Pattern: spread_below_threshold:Xbps<Ybps[stageN:label]
        const spreadMatch = e.rejectionReason.match(/spread_below_threshold:(\d+)bps<(\d+)bps(?:\[([^\]]+)\])?/);
        if (!spreadMatch)
            continue;
        ...
        const tradeSizeUi = meta['tradeSizeUi'] ?? 0.5;
...
function simulateConfig(obs, cfg, hoursOfData) {
 ...
 */
export function runBacktest(events, tradeSizeUi = 0.5, solPriceUsd = 150, hoursOfData = 1) {
    ...
    for (const minSpreadBps of minSpreadRange) {
        for (const minProfitUsd of minProfitRange) {
            configs.push(simulateConfig(observations, { minSpreadBps, minProfitUsd, tradeSizeUi, solPriceUsd }, hoursOfData));
        }
    }
    ...
    if (observations.length === 0) {
        recommendations.push('No spread observations in journal yet. Let the bot run for at least 30 minutes to collect data.');
    }
    else {
        const maxSpread = Math.max(...observations.map((o) => o.spreadBps));
        const medianSpread = observations.sort((a, b) => a.spreadBps - b.spreadBps)[Math.floor(observations.length / 2)]?.spreadBps ?? 0;
        ...
        recommendations.push(`Break-even spread at ${tradeSizeUi} SOL / $${notionalUsd.toFixed(0)} notional: ${breakEvenBps} bps.`);
        recommendations.push(`Observed max spread: ${maxSpread} bps, median: ${medianSpread} bps.`);
        recommendations.push(`${pctAbove}% of observations (${aboveBreakEven}/${observations.length}) are above break-even.`);
        if (maxSpread < breakEvenBps) {
            recommendations.push(`⚠ ALL observed spreads are below break-even. Core pairs alone cannot cover tx costs at ${tradeSizeUi} SOL. ` +
            ...
            recommendations.push(`Optimal config: minSpreadBps=${best.config.minSpreadBps}, minProfitUsd=$${best.config.minProfitUsd} ` +
            ...
            if (best.config.minSpreadBps < currentThreshold) {
                recommendations.push(`Current threshold (${currentThreshold} bps) is ${thresholdGapBps} bps above optimal. ` +
        ...
        if (aboveBreakEven === 0 && maxSpread > 0) {
            recommendations.push(`Strategy is working correctly — it's correctly filtering unprofitable spreads. ` +
