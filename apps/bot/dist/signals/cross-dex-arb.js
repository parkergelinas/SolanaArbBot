...
export const CROSS_DEX_CORE_PAIRS = [
...
export const DEFAULT_CROSS_DEX_CONFIG = {
...
export class CrossDexArbStrategy {
    client;
    tradeAmountUi;
    cfg;
    pumpRegistry;
    ...
    corePairIndex = 0;
    lastOpportunities = [];
    constructor(client, tradeAmountUi, cfg = DEFAULT_CROSS_DEX_CONFIG, pumpRegistry = null) {
        this.client = client;
    ...
    async scan(state) {
        ...
        if (batch.length === 0)
            return state;
        try {
            const result = await scanCrossDexPairs(this.client, batch, this.tradeAmountUi, { slippageBps: 50, concurrency: this.cfg.scanConcurrency });
