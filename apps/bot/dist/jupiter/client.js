...
export function atomicToUi(amount, decimals) {
...
export function uiToAtomic(amountUi, decimals) {
    const scale = 10 ** decimals;
    return BigInt(Math.floor(amountUi * scale));
}
export function parsePriceImpactPct(raw) {
    if (raw === undefined)
        return 0;
    const n = typeof raw === 'number' ? raw : Number.parseFloat(raw);
    return Number.isFinite(n) ? Math.abs(n) : 0;
}
export function routeHopCount(quote) {
