 ...
 */
export async function checkPnlAlert(totalPnlUsd, solPriceUsd) {
    ...
    if (totalPnlUsd < thresholdUsd) {
        await sendAlert(`⚠️ *PnL alert*: session PnL is *${totalPnlUsd.toFixed(4)} USD* (threshold ${thresholdUsd.toFixed(4)} USD)`);
    }
}
export async function alertHalt(reason) {
