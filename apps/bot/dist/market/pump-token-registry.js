...
function scorePair(pair) {
    ...
    if (liq < MIN_LIQUIDITY_USD || liq > MAX_LIQUIDITY_USD)
        return 0;
    if (vol24h < MIN_VOLUME_24H_USD)
        return 0;
    if (ageDays > MAX_PAIR_AGE_DAYS)
        return 0;
...
async function fetchCandidates() {
    ...
    const pumpSearch = await fetchJson(`${DS_BASE}/latest/dex/search?q=pump`);
    if (pumpSearch?.pairs) {
    ...
    const profiles = await fetchJson(`${DS_BASE}/token-profiles/latest/v1`);
    ...
    if (Array.isArray(profiles)) {
        ...
        if (solanaMints.length > 0) {
            ...
            for (let i = 0; i < solanaMints.length; i += chunkSize) {
                ...
                const tokenData = await fetchJson(`${DS_BASE}/latest/dex/tokens/${chunk.join(',')}`);
                if (tokenData?.pairs) {
