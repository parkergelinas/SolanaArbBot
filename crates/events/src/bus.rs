...
/// Default bounded channel size per subscriber.
pub const DEFAULT_CHANNEL_CAPACITY: usize = 4_096;

/// Subscriber identifier assigned by the event bus.
pub type SubscriberId = u64;

/// Backpressure behaviour when a subscriber channel is full.
...
impl EventBusConfig {
    ...
    /// Returns configured backpressure behaviour.
    #[must_use]
...
impl EventBus {
    ...
    ///
    /// With [`BackpressureStrategy::Block`] this method may block.
    /// Async code should call [`Self::publish_async`] instead.
...
impl EventSubscriber {
    ...
    /// Blocks up to `timeout` waiting for the next event.
...
mod tests {
    ...
    use super::{BackpressureStrategy, EventBus, EventBusConfig};
    ...
    fn event_bus_fans_out_to_multi_consumer() {
        ...
        assert_eq!(first.recv().expect("first"), event);
        assert_eq!(second.recv().expect("second"), event);
    }
    ...
    #[tokio::test]
    async fn event_bus_async_publish_delivers() {
        let bus = EventBus::new();
        let subscriber = bus.subscribe();
        let event = pool_event();

        let report = bus.publish_async(event.clone()).await.expect("async publish");
